                    ...
                    <div class="tables">
                        ...
                        <div class="panel-body widget-shadow">
                            ...
                            <table id="table2" class="table">
                                ...
                                <tbody>
                                    <?php 
                                    $user=mysqli_query($connect,"SELECT * FROM user_mst WHERE usertype_id=3");
                                    while($supp= mysqli_fetch_array($user))
                                    {
                                    $userid=$supp['user_id'];
                                     $query = mysqli_query($connect,"SELECT * FROM order_mst WHERE by_id = $userid");
                                     while($row = mysqli_fetch_assoc($query))
                                     {
                                         $q2 = mysqli_query($connect,"SELECT * FROM user_mst WHERE user_id = '{$row['by_id']}'");
                                         while($result = mysqli_fetch_assoc($q2))
                                         {
                                    ?>
                                    <tr>
                                        <td><?php echo $row['ord_id'] ; ?></td>
                                        <td><?php echo $result['f_name'] ." ". $result['m_name'] . $result['l_name'];?></td>
                                        <td><?php echo $row['order_date_time']; ?></td>
                                        <td><?php echo $row['total']; ?></td>
                                        <td><?php echo $row['shipname'] ."<br>".$row['shipnumber']."<br>".$row['address']  ; ?></td>
...
{
     echo "<option value='cust_order.php?id={$row['ord_id']}'>Pending</option>"; 
}
 elseif($row['o_status']=='Pending') {
     echo "<option value='cust_order.php?id={$row['ord_id']}'>Delivered</option>"; 
}
