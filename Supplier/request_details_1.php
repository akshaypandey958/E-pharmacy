...
if (!isset($_SESSION['id'])) {
 ...
 if(isset($_GET['cid']))
    ...
    window.location.href = 'request_details.php';
    </script>";
            }
         }
?>
         ...
         <div style=" box-shadow: 0px 0px 15px 0px #D6D6D6;padding-left:80px;padding-bottom:160px;padding-top:160px;padding-right:80px;margin-bottom:15px;margin-top:15px;width:100%;height: 50%; ">
                        ...
                        <thead style="background: #CACACA;font-size:20px;">
                            <tr id="agileinfo-nav_search">
                                ...
                                <td class="invert">Available Quantity</td>
             ...
             <?php
             $check=mysqli_query($connect, "SELECT * FROM order_mst WHERE ord_id='{$oid}'");
             $status= mysqli_fetch_array($check);
             $type=$status['type'];
             $status1=$status['o_status'];
                     if ($type=='Req' AND $status1=='Pending')
                     {?>
             <br><br><h4 class="" style="font-size:20px;text-align:left;margin-bottom: 50px;"><?php echo "<a style='color: red;' href='request_details_1.php?cid=$oid'>Request Cancel ??</a>"?>
